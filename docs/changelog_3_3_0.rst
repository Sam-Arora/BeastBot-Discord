...
- Converting from and to Postgres driver with ``beastbot-setup convert`` have been fixed (:issue:`3714`, :issue:`3115`)
...
- Added `beastbot.core.utils.AsyncIter` utility class which allows you to wrap regular iterable into async iterator yielding items and sleeping for ``delay`` seconds every ``steps`` items (:issue:`3767`, :issue:`3776`)
...
- `beastbot.core.utils.common_filters.filter_invites` now filters ``discord.io/discord.li`` invites links (:issue:`3717`)
- Fixed false-positives in `beastbot.core.utils.common_filters.filter_invites` (:issue:`3717`)
...
- **Core Bot** - ``beastbot-setup`` now prints link to Getting started guide at the end of the setup (:issue:`3027`)
...
- **Utility Functions** - `beastbot.core.utils.menus.menu()` now checks permissions *before* trying to clear reactions (:issue:`3589`, :issue:`3145`)


beastbot 3.3.2 (2020-02-28)
=========================
...
- Add clearer error when page is of a wrong type in `beastbot.core.utils.menus.menu()` (:issue:`3571`)
...
- Allow to edit prefix from command line using ``beastbot --edit``.
