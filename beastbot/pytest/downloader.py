...
from beastbot.cogs.downloader.repo_manager import RepoManager, Repo, ProcessFormatter
from beastbot.cogs.downloader.installable import Installable, InstalledModule
...
async def _session_git_repo(tmp_path_factory, event_loop):
    ...
    repo = Repo(name="beastbot-testrepo", url="", branch="master", commit="", folder_path=repo_path)
...
async def git_repo(_session_git_repo, tmp_path, event_loop):
    ...
    repo_path = tmp_path / "beastbot-testrepo"
    shutil.copytree(_session_git_repo.folder_path, repo_path)
    repo = Repo(
        name="beastbot-testrepo",
...
async def cloned_git_repo(_session_git_repo, tmp_path, event_loop):
    ...
    repo_path = tmp_path / "beastbot-cloned_testrepo"
    repo = Repo(
        name="beastbot-testrepo",
...
async def git_repo_with_remote(git_repo, tmp_path, event_loop):
    ...
    repo_path = tmp_path / "beastbot-testrepo_with_remote"
    repo = Repo(
        name="beastbot-testrepo",
