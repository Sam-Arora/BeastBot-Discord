...
Anything in the ``beastbot.core`` module or any of its submodules 
which is not private (even if not documented) should not break without notice.

Anything in the ``beastbot.cogs`` and ``beastbot.vendored`` modules or any of their submodules is specifically
