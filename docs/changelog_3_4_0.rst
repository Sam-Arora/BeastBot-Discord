...
- **Core Bot** - Added info about the metadata file to ``beastbot --debuginfo`` (:issue:`4557`)
...
4. Information for Audio users that are using an external Lavalink instance (if you don't know what that is, you should skip this point):
   ...
   We've updated our `application.yml file <https://github.com/Cog-Creators/Red-DiscordBot/blob/3.4.1/beastbot/cogs/audio/data/application.yml>`__ and you should update your instance's ``application.yml`` appropriately.
...
- Added API for setting contextual locales (:issue:`3896`, :issue:`1970`)
    ...
    - New function added: `beastbot.core.i18n.set_contextual_locales_from_guild()`
...
- Moved ``beastbot.core.checks.bot_in_a_guild()`` to `beastbot.core.commands.bot_in_a_guild()` (old name has been left as an alias) (:issue:`4515`, :issue:`4510`)
...
- Deprecated ``beastbot.core.utils.mod.is_allowed_by_hierarchy`` (:issue:`4435`)
...
- Added `beastbot.core.utils.get_end_user_data_statement()` and `beastbot.core.utils.get_end_user_data_statement_or_raise()` to attempt to fetch a cog's End User Data Statement (:issue:`4404`)
- Added `beastbot.core.utils.chat_formatting.quote()` to quote text in a message (:issue:`4425`)
...
- Removed things past deprecation time: (:issue:`4163`)
    ...
    - ``beastbot.core.commands.APIToken``
