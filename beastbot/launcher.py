...
from beastbot import MIN_PYTHON_VERSION
from beastbot.setup import (
    basic_setup,
...
from beastbot.core import __version__, version_info as red_version_info, VersionInfo
...
def run_red(selected_instance, autorestart: bool = False, cliflags=None):
    ...
    while True:
        ...
        cmd_list = [interpreter, "-m", "beastbot", selected_instance]
        if cliflags:
            cmd_list += cliflags
...
def main_menu(flags_to_pass):
    if IS_WINDOWS:
        os.system("TITLE BeastBot Launcher")
    clear_screen()
    while True:
        print(INTRO)
